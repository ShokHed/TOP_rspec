...
describe Dog do
  ...
  subject(:yana) { described_class.new('yana', 'akita', 'spotted') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when described class has method name shared with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(yana).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(yana).to be_sleeping
  end
end
