...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        ...
        valid_input = '8'
        allow(game_input).to receive(:gets).and_return(invalid_input_one, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        min = game_verify.instance_variable_get(:@minimum)
        max = game_verify.instance_variable_get(:@maximum)
        bad_input = game_verify.verify_input(min, max, invalid_input)
        expect(bad_input).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        max_guesses = game_five_hundred.maximum_guesses
        expect(max_guesses).to eql(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      # start_guesses = game_turns.instance_variable_get(:@guess_count)
      expect { game_turns.display_turn_order(search_turn) }.to change { game_turns.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_turn).to receive(:make_guess).once
      game_turns.display_turn_order(search_turn)
    end
